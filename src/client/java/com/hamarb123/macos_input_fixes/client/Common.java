...
public class Common
{
	public static boolean hasControlDownInjector()
	{
		...
		else
		{
			...
			returnValue = FabricReflectionHelper.Screen_hasControlDown() ||
				//ctrl key check
				FabricReflectionHelper.InputUtil_isKeyPressed_1(((MinecraftClientAccessor)Minecraft.getInstance()).getWindow().handle(), 341) ||
				FabricReflectionHelper.InputUtil_isKeyPressed_1(((MinecraftClientAccessor)Minecraft.getInstance()).getWindow().handle(), 345);
		}
	...
	public static final boolean IS_SYSTEM_MAC = System.getProperty("os.name", "").toLowerCase(java.util.Locale.ROOT).contains("mac");
}
